...
#first u need to make this script executable with this command #chmod +x MNDWI.sh
#This script calculates Modified Normalized Difference Water Index
#usage: ./MNDWI.sh

echo "Calculate Sentinel2 MNDWI"
echo ""
...
gdal_calc.py -A $sds_names --A_band=2 -B $sds_names --B_band=9  --outfile=$output  --calc="(A.astype(float)-B)/(A.astype(float)+B)" --type='Float32'
